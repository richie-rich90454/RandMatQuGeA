...
export function generateIntegrationAdvanced(difficulty?: string, rng: RngFn=Math.random): QuestionDto{
	...
	switch(questionType){
		...
		case "volumeDiscOther":{
			...
			choices.push((correctNum+1).toFixed(2));
			choices.push((correctNum-1).toFixed(2));
			choices.push((Math.PI*(Math.pow(a,5)/5 + Math.pow(a,3)/3 + a)).toFixed(2));
			choices.push((Math.PI*(Math.pow(a,5)/5 + a)).toFixed(2));
			break;
		}
		case "volumeWasher":{
			let a=Math.floor(rng()*maxCoeff)+1;
			...
			choices.push((correctNum+1).toFixed(2));
			choices.push((correctNum-1).toFixed(2));
			choices.push((Math.PI*Math.pow(intersect,5)/3).toFixed(2));
			choices.push((Math.PI*Math.pow(intersect,5)/5).toFixed(2));
			break;
		}
		...
		case "improperVerticalAsymptote":{
			...
			latexAnswer=`\\arcsin\\left(\\frac{1}{${a}}\\right)`;
			expectedFormat="Enter number or arcsin(1/a)";
