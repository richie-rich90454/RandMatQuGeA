...
export function generateArcLength(difficulty?: string): void{
	...
	window.correctAnswer={ correct: arc, alternate: arc };
...
export function generateAngularLinearSpeed(difficulty?: string): void{
	...
	if (type==="angular"){
		...
		window.correctAnswer={ correct: omega.toFixed(2), alternate: omega.toFixed(2) };
	}
    else{
		questionArea.innerHTML=`A wheel of radius ${r} m rotates at ${rpm} rpm. Find the linear speed of a point on its rim in m/s.`;
		window.correctAnswer={ correct: v.toFixed(2), alternate: v.toFixed(2) };
	}
...
export function generateElevationDepression(difficulty?: string): void{
	...
	if (type==="elevation"){
		...
		window.correctAnswer={ correct: height.toFixed(2), alternate: height.toFixed(2) };
	}
    else{
		...
		window.correctAnswer={ correct: distance.toFixed(2), alternate: distance.toFixed(2) };
	}
...
export function generateReferenceAngle(difficulty?: string): void{
	...
	window.correctAnswer={ correct: ref.toString(), alternate: ref+"°" };
...
export function generateASTCSign(difficulty?: string): void{
	...
	window.correctAnswer={ correct: sign, alternate: sign };
...
export function generateSumDifference(difficulty?: string): void{
	...
	window.correctAnswer={
		correct: value.toFixed(4),
		alternate: value.toFixed(4)
	};
...
export function generatePolarDistance(difficulty?: string): void{
	...
	window.correctAnswer={ correct: dist, alternate: dist };
...
export function generatePolarGraphEquation(difficulty?: string): void{
	...
	window.correctAnswer={ correct: description, alternate: description };
...
export function generateParametricToCartesian(difficulty?: string): void{
	...
	window.correctAnswer={ correct: cartesian, alternate: cartesian };
...
export function generateComplexPolarForm(difficulty?: string): void{
	...
	window.correctAnswer={
		correct: `${r} cis ${thetaDeg}°`,
		alternate: `${r}(\\cos ${thetaDeg}° + i \\sin ${thetaDeg}°)`
	};
...
export function generateComplexMultiplyDivide(difficulty?: string): void{
	...
	window.correctAnswer={
		correct: `${resultR} cis ${resultThetaDeg}°`,
		alternate: `${resultR}(\\cos ${resultThetaDeg}° + i \\sin ${resultThetaDeg}°)`
	};
...
export function generateDeMoivre(difficulty?: string): void{
	...
	window.correctAnswer={
		correct: `${newR} cis ${newThetaDeg}°`,
		alternate: `${newR}(\\cos ${newThetaDeg}° + i \\sin ${newThetaDeg}°)`
	};
	window.expectedFormat="Enter as 'r cis θ°' or expanded form";
	if (window.MathJax?.typeset) window.MathJax.typeset();
}
