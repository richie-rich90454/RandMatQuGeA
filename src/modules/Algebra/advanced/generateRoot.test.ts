...
describe("generateRoot",()=>{
	it("returns a QuestionDto with required fields",()=>{
		const dto=generateRoot("medium", seededRng(42));
	...
	it("should handle easy difficulty",()=>{
		const dto=generateRoot("easy", seededRng(42));
	...
	it("should handle hard difficulty",()=>{
		const dto=generateRoot("hard", seededRng(42));
		expect(dto.correct).toBeDefined();
	});
	it("returns deterministic output for same seed",()=>{
		const dto1=generateRoot("medium", seededRng(42));
		const dto2=generateRoot("medium", seededRng(42));
		expect(dto1).toEqual(dto2);
	});
	it("produces different output for different seeds",()=>{
		const dto1=generateRoot("medium", seededRng(42));
		const dto2=generateRoot("medium", seededRng(99));
