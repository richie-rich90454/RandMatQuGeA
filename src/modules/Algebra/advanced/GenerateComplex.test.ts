...
describe("generateComplex",()=>{
	...
	it("should handle easy difficulty",()=>{
		const dto=generateComplex("easy", seededRng(42));
	...
	it("should handle hard difficulty",()=>{
		const dto=generateComplex("hard", seededRng(42));
		expect(dto.correct).toBeDefined();
	});
	it("returns deterministic output for same seed",()=>{
		const dto1=generateComplex("medium", seededRng(42));
		const dto2=generateComplex("medium", seededRng(42));
