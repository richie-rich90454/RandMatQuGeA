 ...
 * @date 2026-04-18
 */
export function generateCartesianConcepts(difficulty?: string, rng: RngFn = Math.random): QuestionDto{
	...
	switch(type){
		case "quadrant":{
			const x=Math.floor(rng()*max*2)-max;
			const y=Math.floor(rng()*max*2)-max;
			let quadrant="";
			...
			alternate=quadrant;
			display=quadrant;
			mathExpression=`In which quadrant is the point \\( (${x}, ${y}) \\)?`;
			...
			expectedFormat="Enter I, II, III, IV, or 'on an axis'";
			break;
		}
		case "distance":{
			const x1=Math.floor(rng()*max);
			const y1=Math.floor(rng()*max);
			const x2=Math.floor(rng()*max);
			const y2=Math.floor(rng()*max);
			const dist=Math.sqrt((x2-x1)**2+(y2-y1)**2).toFixed(2);
			correct=dist;
			alternate=dist;
			display=dist;
			mathExpression=`Find the distance between \\( (${x1}, ${y1}) \\) and \\( (${x2}, ${y2}) \\).`;
			...
			expectedFormat="Enter a decimal rounded to two places";
			break;
		}
		case "midpoint":{
			const x1=Math.floor(rng()*max);
			...
			alternate=`(${mx},${my})`;
			display=displayAns;
			mathExpression=`Find the midpoint of \\( (${x1}, ${y1}) \\) and \\( (${x2}, ${y2}) \\).`;
		...
		case "plot":{
			...
			const y=Math.floor(rng()*max)+1;
			const displayAns=`(${x}, ${y})`;
			correct=displayAns;
			alternate=`(${x},${y})`;
			display=displayAns;
			mathExpression=`What are the coordinates of the point that is ${x} units right and ${y} units up from the origin?`;
			choices=[correct];
			choices.push(`(${x+1}, ${y})`);
			choices.push(`(${x}, ${y+1})`);
			choices.push(`(${x-1}, ${y})`);
