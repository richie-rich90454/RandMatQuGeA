 ...
 * @date 2026-04-18
 */
export function generateExponentialModeling(difficulty?: string, rng: RngFn = Math.random): QuestionDto{
	...
	let mathExpression="";
	let choices:string[]=[];
	const initial=Math.floor(rng()*max*10)+50;
	const rate=(rng()*0.1+0.05).toFixed(3);
	const time=Math.floor(rng()*5)+1;
	switch(type){
		case "growth":{
		...
		case "half-life":{
			const halfLife=Math.floor(rng()*max)+5;
		...
		case "cooling":{
			...
			const k=(rng()*0.1*(max/5)+0.05).toFixed(3);
			const t=Math.floor(rng()*10)+1;
