...
describe("generateUnitConversion",()=>{
	...
	it("should handle easy difficulty",()=>{
		const dto=generateUnitConversion("easy", seededRng(42));
	...
	it("should handle hard difficulty",()=>{
		const dto=generateUnitConversion("hard", seededRng(42));
		expect(dto.correct).toBeDefined();
	});
	it("returns deterministic output for same seed",()=>{
		const dto1=generateUnitConversion("medium", seededRng(42));
		const dto2=generateUnitConversion("medium", seededRng(42));
		expect(dto1).toEqual(dto2);
	});
	it("produces different output for different seeds",()=>{
		const dto1=generateUnitConversion("medium", seededRng(42));
		const dto2=generateUnitConversion("medium", seededRng(99));
